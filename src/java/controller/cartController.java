 ...
 * @author Fantasy
 */
@WebServlet(name = "cartController", urlPatterns = {"/URLcart"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (service.equals("add2cart")) {
                ...
                if(pro == null){
                    ...
                    pro = new Product(pid, product.getProduct_name(), product.getPrice()
                            , product.getQuantity()-1, product.getDescription(), product.getImage_url(),
                            product.getBrand_id(), product.getRelease_date(),1);
                    session.setAttribute(pid+"", pro);
                    response.sendRedirect("Home");
                }else{
                    pro.setQuantityUser(pro.getQuantityUser()+1);
            ...
            if (service.equals("add2cart2")) {
                ...
                if (service.equals("add2cartStore")) {
                   int pid = Integer.parseInt(request.getParameter("product_id"));
                   Product pro;
                   pro = (Product)session.getAttribute(pid + "");
                   if(pro == null){
                    ...
                    pro = new Product(pid, product.getProduct_name(), product.getPrice()
                            ...
                            product.getBrand_id(), product.getRelease_date(),1);
                    session.setAttribute(pid+"", pro);
                    response.sendRedirect("URLProduct?Type="+service1);
                   }else{
                       pro.setQuantityUser(pro.getQuantityUser()+1);
                       pro.setQuantity(pro.getQuantity()-1);
                       session.setAttribute(pid+"", pro);
