...
<%@page import="java.util.Enumeration,entity.Product,java.util.Vector,java.sql.ResultSet,entity.User"%>
<!DOCTYPE html>
<html lang="en">
    <%
    ...
    <body>
            ...
            <div id="top-header">
                <div class="container">
                    <ul class="header-links pull-left">
                        <li><a href="#"><i clSass="fa fa-phone"></i>0763221204</a></li>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="header-search">
                                <form action="URLOrder">
                                    <input name="search" class="input" placeholder="Search here">
                                    <a href="URLOrder?manageCus=search"><button class="search-btn" name="user_id" value="${user_id}">Search</button></a>
                                    <input type="hidden" name="manageCus" value="search" ></td>
                                </form>
                            </div>
                        </div>
                        <!-- /SEARCH BAR -->
        ...
        <div class="container">
            <div class="table-wrapper">
                    ...
                    <tbody>
                        <%while(rs.next()){%>
                            <tr>
                                ...
                                <td><%=rs.getInt(3)%></td>
                                <td><%=rs.getDouble(4)*rs.getInt(3)%></td>
                                <td><%=rs.getString(5)%></td>
                                <%if(!rs.getString(5).equals("Cancelled")){
                                  if(user.getIsAdmin()==1){
                                %>
                                <td>
                                    <a href="URLOrder?manageCus=Delivered&item_id=<%=rs.getInt(6)%>&user_id=<%=rs.getInt(7)%>">Process</a>
                                </td>
                                <td>
                                    <a href="URLOrder?manageCus=Delivered&item_id=<%=rs.getInt(6)%>&user_id=<%=rs.getInt(7)%>">Delivered</a>
                                </td>
                                <td>
                                    <a href="URLOrder?manageCus=Complete&item_id=<%=rs.getInt(6)%>&user_id=<%=rs.getInt(7)%>">Shipped</a>
                                </td>
                                
                                <%}%>
                                <td>
                                    <%String manageCus= (String)request.getAttribute("manageCus");
                                    if(manageCus!=null && !manageCus.equals("Complete")){%>
                                       <a href="URLOrder?manageCus=Cancelled&item_id=<%=rs.getInt(6)%>&user_id=<%=rs.getInt(7)%>">Cancel</a>
                                       <%}%>
                                </td>
                                <%}%>
                            </tr>
             ...
             <c:if test="${sessionScope.acc.getIsAdmin() ==q}">
                <a href="URLOrder?manageCus=listCustomer"><button type="button"">List of order</button>
            </c:if>
        </div>
